...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['confirm_submit'])) {
    ...
    try {
        ...
        $month_check = checkVisitLimit($db, $customer_ids, $visit_type, 'month', $visits_per_month, $visit_date);
        if (!$month_check['allowed']) {
            throw new Exception("Monthly food visit limit reached. This customer has {$month_check['count']} food visits this month (limit: " . formatLimitText($month_check['limit']) . ").");
        }
        
        $year_check = checkVisitLimit($db, $customer_ids, $visit_type, 'year', $visits_per_year, $visit_date);
        if (!$year_check['allowed']) {
            throw new Exception("Yearly food visit limit reached. This customer has {$year_check['count']} food visits this year (limit: " . formatLimitText($year_check['limit']) . ").");
        }
        
        $days_check = checkMinDaysBetween($db, $customer_ids, $visit_type, $min_days_between, $visit_date);
        if (!$days_check['allowed']) {
            throw new Exception("Minimum {$days_check['min_days']} days required between food visits. Last food visit was {$days_check['days_since']} days ago.");
        }
...
<script>
initCustomerSearch('customer_search', 'customer_id', 'customer_results', 'food', 'eligibility_errors');
initDateTimeOverride('override_visit_date', 'auto_visit_datetime', 'manual_visit_datetime', 'manual_visit_datetime_input');

<?php if ($customer): ?>
checkEligibility(<?php echo $customer['id']; ?>, 'food', 'eligibility_errors');
<?php endif; ?>
</script>
